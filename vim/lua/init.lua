...
opt.ruler = true -- Show cursor position (Kept from first block)
...
require("lazy").setup({
	...
	-- formatter
	{
		...
		opts = {
			...
			-- One-liner per filetype; Conform picks the first that exists on $PATH
			formatters_by_ft = {
				lua = { "stylua" },
				python = { "black", "isort" },
				javascript = { "prettier" },
	...
	-- optional: auto-install the binaries you list above
	{ "zapling/mason-conform.nvim", dependencies = { "williamboman/mason.nvim" } },
	...
	-- diagnostics / lint messages
	{
		...
		config = function()
			...
			-- trigger on write or CursorHold
	...
	{ "skywind3000/asyncrun.vim" }, -- Run async commands
	{
		...
		build = "~/.asdf/shims/npm install -g mcp-hub@latest", -- Installs `mcp-hub` node binary globally
		config = function()
